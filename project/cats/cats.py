...
def about(subject):
    ...
    def judge(str):
        ...
        for element0 in subject:
             for element1 in str_words:
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    typed_size = len(typed_words)
    source_size = len(source_words)

    if(typed_size == 0 and source_size == 0):
        return 100.0
    elif(typed_size == 0 or source_size == 0):
        return 0.0

    cnt = 0

    for i in range(0, typed_size):
        if(i < source_size and typed_words[i] == source_words[i]):
            cnt += 1
    
    return cnt / typed_size * 100.0
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        differ = diff_function(typed_word, word, limit)
        if(differ < mindiffer):
            mindiffer = differ
            ans = word
